...
class EnhancedTestLauncher(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Create tab widget and manager for main content
        if _launcher_modules_available:
            # Use extracted TabManager for modern tab management
            main_tabs = QTabWidget()
            self.tab_manager = TabManager(main_tabs)
            
            # Register gallery tab (using wrapped component)
            ...
            # Register extracted tab components
            ...
            # Fallback for when extracted modules not available
            try:
                ...
                main_tabs = QTabWidget()
                self.tab_manager = None  # No manager in fallback
                
                # Gallery tab (using fallback component)
                ...
                # Final fallback if no tabs available
                layout.addWidget(self.gallery_widget)
                
            # Set up status area for fallback
            self.status_area = QTextEdit()
            self.status_area.setReadOnly(True)
            self.status_area.setMaximumHeight(100)
            self.status_area.setStyleSheet("background-color: #f8f9fa;")
