...
class ComprehensiveDeltaRecorder:
    ...
    def __init__(self, output_path: str):
        ...
        # Smart utility tracking - only calculate when bundles change
        self._last_agent_bundles: Dict[int, Tuple[float, float]] = {}  # agent_id -> (good1_total, good2_total)
        self._utility_change_agents: set = set()  # Agents with bundle changes this step
    ...
    def record_initial_state(self, simulation: Simulation) -> None:
        ...
        for agent in simulation.agents:
            ...
            # Initialize bundle tracking for smart utility calculation
            carrying = getattr(agent, 'carrying', {})
            home = getattr(agent, 'home_inventory', {})
            initial_bundle = (
                float(carrying.get("good1", 0) + home.get("good1", 0)),
                float(carrying.get("good2", 0) + home.get("good2", 0))
            )
            self._last_agent_bundles[agent.id] = initial_bundle
    ...
    def start_step_recording(self, step: int) -> None:
        ...
        # Clear utility change tracking for this step
        self._utility_change_agents.clear()
    ...
    def _record_agents_single_pass(self, simulation: Simulation, step: int) -> tuple:
        """Record all agent-related changes in a single iteration for maximum efficiency.
        
        Returns:
            Tuple of (visual_delta, agent_moves, agent_mode_changes, agent_inventory_changes, agent_target_changes, agent_utility_changes)
        """
        ...
        for agent in simulation.agents:
            ...
            # Get current inventory state (fast dictionary access)
            carrying = getattr(agent, 'carrying', {})
            home = getattr(agent, 'home_inventory', {})
            current_carrying = dict(carrying)
            current_home = dict(home)
            
            # Get current bundle for utility tracking
            current_bundle = (
                float(carrying.get("good1", 0) + home.get("good1", 0)),
                float(carrying.get("good2", 0) + home.get("good2", 0))
            )
            ...
            if agent_id in self._last_agent_positions:
                ...
                if current_pos != last_pos:
                    ...
                    # Also add to visual delta
                    visual_moves.append((agent_id, last_pos[0], last_pos[1], current_pos[0], current_pos[1]))
            
            # Check for mode changes
            if agent_id in self._last_agent_modes:
                last_mode = self._last_agent_modes[agent_id]
                if current_mode != last_mode:
                    # Mark for utility recalculation (mode changes can affect utility)
                    self._utility_change_agents.add(agent_id)
                    
                    # Calculate utility before/after using real simulation method
                    utility_before = self._last_agent_utilities.get(agent_id, 0.0)
                    utility_after = agent.current_utility()  # Use real utility calculation
                    
                    agent_mode_changes.append(AgentModeChange(
                        agent_id=agent_id,
                        old_mode=last_mode,
            ...
            if agent_id in self._last_agent_inventories:
                ...
                if current_carrying != last_carrying or current_home != last_home:
                    # Inventory changed - mark for utility recalculation
                    self._utility_change_agents.add(agent_id)
                    
                    # Create separate inventory changes for carrying and home
                    if current_carrying != last_carrying:
                        agent_inventory_changes.append(InventoryChange(
                            agent_id=agent_id,
                            inventory_type="carrying",
                            old_inventory=last_carrying,
                            new_inventory=current_carrying,
                            change_type="collection"
                        ))
            ...
            if agent_id in self._last_agent_targets:
                ...
                if current_target != last_target:
                    agent_target_changes.append(TargetChange(
                        agent_id=agent_id,
            ...
            # Check for utility changes (only if bundle changed)
            if agent_id in self._last_agent_bundles:
                last_bundle = self._last_agent_bundles[agent_id]
                if current_bundle != last_bundle:
                    # Bundle changed - calculate utility using real simulation method
                    current_utility = agent.current_utility()  # Use real utility calculation
                    last_utility = self._last_agent_utilities.get(agent_id, 0.0)
                    
                    if abs(current_utility - last_utility) > 0.001:  # Avoid floating point precision issues
                        agent_utility_changes.append(UtilityChange(
                            agent_id=agent_id,
                            old_utility=last_utility,
                            new_utility=current_utility,
                            utility_delta=current_utility - last_utility,
                            calculation_context="bundle_change"
                        ))
            ...
            # Check for state changes (carrying status)
            if agent_id in self._last_agent_states:
                last_carrying_state = self._last_agent_states[agent_id]
                current_carrying_state = any(count > 0 for count in carrying.values()) if carrying else False
                
                if current_carrying_state != last_carrying_state:
                    visual_state_changes.append((agent_id, current_carrying_state))
    ...
    def _record_agent_utility_changes(self, simulation: Simulation, step: int) -> List[UtilityChange]:
        """Record agent utility calculation events - only when bundles actually change."""
        utility_changes = []
        
        for agent in simulation.agents:
            agent_id = agent.id
            
            # Get current bundle (fast - just dictionary access)
            carrying = getattr(agent, 'carrying', {})
            home = getattr(agent, 'home_inventory', {})
            current_bundle = (
                float(carrying.get("good1", 0) + home.get("good1", 0)),
            ...
            # Only calculate utility if bundle changed
            if agent_id in self._last_agent_bundles:
                last_bundle = self._last_agent_bundles[agent_id]
                if current_bundle != last_bundle:
                    # Bundle changed - calculate utility using real simulation method
    ...
    def save_deltas(self) -> None:
        ...
        # Check if disk writing is disabled via environment variable
        if os.environ.get('ECONSIM_DISABLE_DELTA_SAVE') == '1':
            print(f"💾 Skipped saving {len(self.deltas)} comprehensive deltas (ECONSIM_DISABLE_DELTA_SAVE=1)")
            return
